...
class VJNObject:
    ...
    def set_composition_menu(self, id, disabled : bool):
        return Interaction()\
            .add_button(label = "Pate", id = f"pate-{id}")\
            .add_button(label = "Garniture", id = f"garniture-{id}")\
            .add_interaction(
                Interaction()
                    .add_button(label = "Valider", style = Style.GREEN, id = f"valider-composition-{id}", disabled = disabled)
                    .add_button(label = "Retour", style = Style.GREY, id = f"retour-start-{id}")
                    .add_button(label = "Annuler", style = Style.RED, id = f"annuler-composition-{id}")
            )

    def set_pate(self, id, default : list[str]):
        # Récupération recette existante
        self.database.execute("SELECT * FROM ingredient_VJN").fetchall()

        # Création composent
        menu = [{"label": f"{self.database[product, 'name'].capitalize()} - {self.database[product, 'price'] if self.database[product, 'price'] != '0,00 €' else 'Gratuit'}", "value": f"ingredient-{product}", "default": int(product) in default} for product in [str(product) for product in self.json.get("compose").values()] if int(product) in self.database and self.database[product, 'type']]

        start_menu = Interaction().add_menu(id = f"menu-pate-{id}", placeholder = "Choisissez votre pate")
        self.set_menu(start_menu, menu)
        return start_menu.add_interaction(
            Interaction()
                .add_button(label = "Retour", style = Style.GREY, id = f"retour-composition-{id}")
                .add_button(label = "Annuler", style = Style.RED, id = f"annuler-composition-{id}")
        )

    def set_garniture(self, id, default : list[int]):
        # Récupération recette existante
        self.database.execute("SELECT * FROM ingredient_VJN").fetchall()
        ...
        start_menu = Interaction().add_menu(id = f"menu-garniture-{id}", placeholder = "Choisissez votre garniture", max = max(1, min(len(menu), 25)))
        self.set_menu(start_menu, menu)
        return start_menu.add_interaction(
            Interaction()
                .add_button(label = "Retour", style = Style.GREY, id = f"retour-composition-{id}")
    ...
    def set_compose_menu(self, id, default : list[int]):
        # Récupération recette existante
        self.database.execute("SELECT * FROM ingredient_VJN").fetchall()
        ...
        return Interaction()\
            .add_interaction(
                Interaction()
                    ...
                    .add_button(label = "Annuler", style = Style.RED, id = f"annuler-{origin}-{id}")
            )
