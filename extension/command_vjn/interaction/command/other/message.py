...
def menu(id_command, bot):
    ...
    menu = [{"label": f"{database[str(product), 'name'].capitalize()} - {database[str(product), 'price'] if database[str(product), 'price'] != '0,00 €' else 'Gratuit'}", "value": f"product-{product}"} for product in json.get("other", []).values() if product in database]
